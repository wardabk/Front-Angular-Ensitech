  ...
  onSubmit(): void {
    ...
    if (this.enseignantForm.valid) {
      ...
      if (this.isEditMode && this.selectedEnseignant) {
        const index = this.enseignants.findIndex(e => e.id === this.selectedEnseignant.id);
        if (index !== -1) {
          this.enseignants[index] = { ...enseignant, id: this.selectedEnseignant.id };
        }
        this.isEditMode = false;
      } else {
        // Ajout d'un nouvel enseignant
        const newId = this.enseignants.length ? this.enseignants[this.enseignants.length - 1].id + 1 : 1;
